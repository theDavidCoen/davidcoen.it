...
var api = "https://localmonero.co/blocks/api/";
...
function checkTx(isFundingTx){
    ...
    if (res.statusText !== "success"){
    	resultsTag.innerHTML = "<span class='validNo'>Failed to get transaction data! Perhaps LocalMonero is down?</span>";
    ...
    if (res.transaction_data === null){
    	resultsTag.innerHTML = "<span class='validNo'>Your transaction exists, but we failed to get its data! LocalMonero API probably has not parsed it yet.</span>";
        throw "Failed to get transaction data!";
    }
...
function queryKeyImage(){
    ...
    if (res.statusText !== "success"){
    	keyImageResultsTag.innerHTML = "<span class='validNo'>Failed to get key image data! Perhaps LocalMonero is down?</span>";
        return;
    }
    res = JSON.parse(res.responseText);
    if (res.status !== "OK"){
